---
title: "230403Visualisation_Chengdu"
...
# Read data
```{r basic clean}
chengdu.raw <- read_tsv("./datafile/Chengdu_data.tsv", col_names = TRUE,show_col_types = FALSE)
chengdu.clean <- data.clean(chengdu.raw)

# Add a new variable for the last word in the sentence
## Note: chengdu condition 3 needs modification! the last sentence is not part of the word
chengdu.clean <- chengdu.clean %>%
  group_by(filename) %>%
  mutate(
...
```{r transform}
chengdu.long_initial <- chengdu.clean %>%
...
chengdu.wide <- chengdu.long_initial %>% 
...
chengdu.long.sen <- chengdu.long %>%
  ...
  mutate(
    ...
    tone = factor(tone, levels = c("A","B","C","D"), labels = c("T1","T2","T3","T4")),
...
write.csv(chengdu.long.sen, "./clean/Chengdu_long_sen.csv", row.names = FALSE)
```
